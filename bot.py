...
async def daily_backup_job(context: ContextTypes.DEFAULT_TYPE):
    if not OWNER_CHAT_ID:
        logger.warning("OWNER_CHAT_ID not set, skipping auto-backup")
        return
    try:
        ...
        if filepath:
            ...
            await safe_send_document(
                context, OWNER_CHAT_ID, filepath,
                caption=f"📦 Daily Backup — {date_str}\n\nملف CSV بكل الطلبات المسجلة."
            )
            logger.info(f"Daily backup sent: {filepath}")
    except Exception as e:
